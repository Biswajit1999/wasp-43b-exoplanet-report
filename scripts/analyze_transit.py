"""Transit-window analysis of a real TESS SPOC light curve for WASP-43 b.
...
DATA_FILE = DATA_DIR / "tess2019058134432-s0009-0000000036734222-0139-s_lc.fits"
PLANET = "WASP-43 b"
PERIOD_DAYS = 0.813475
EPOCH_BJD = 2455528.86774
DURATION_HOURS = 1.1592
SECTOR = 9
FIGURE_FILE = FIG_DIR / "wasp43b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#2b8a3e", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#2b8a3e", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#2b8a3e", alpha=0.08)
